...
Полная пересборка — `python automation/clean.py`: удаляет всё, что перечислено
в `.gitignore` (`build/`, `build-asan/`, `__pycache__/`); `--dry-run` только
показывает список.
...
├── profiles/                   # профили Conan: windows-msvc-*, linux-gcc-*
...
## Новый проект по образцу

Инженерная база проекта — структура, сборка, зависимости, скрипты,
форматирование, статический анализ, санитайзеры, документация, README и CI —
описана в [`SCAFFOLD.md`](SCAFFOLD.md). Имя проекта обозначено в спецификации
как `<Name>` и подставляется при генерации, поэтому по ней собирается любой
новый проект, а не только этот.

Спецификацию выполняет скилл `.claude/skills/new-cpp-project/SKILL.md`. Он
виден из каталога проекта; вызов:

```powershell
/new-cpp-project <Name> <путь>
```

Например, `/new-cpp-project Sandbox ../sandbox` создаёт проект `Sandbox` в
каталоге `sandbox` рядом с этим. Единственный источник содержимого для скилла
— `SCAFFOLD.md`: файлы этого репозитория образцом не считаются. После
генерации скилл проверяет результат сам — сборка, тесты, форматирование,
`clang-tidy`, санитайзеры, документация — и делает первый коммит. Если
проверка не проходит из-за спецификации, скилл останавливается и сообщает об
этом: `SCAFFOLD.md` правит человек.
